...
class ConnectionHeartbeat {
    ...
    int getSRtt() {
        return (int)Clock.nanosToMillis(sRtt);
    }

    int getSRttMicros() {
        return (int)Clock.nanosToMicros(sRtt);
    }
...
//        Logger.info("New SRTT %dms - lastRtt %dus", Clock.microsToMillis(sRtt), Clock.nanosToMicros(lastRtt));
